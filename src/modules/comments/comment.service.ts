...
export default class AmazonCommentService {
  ...
  getAllCommentsByProductId = async (
    ...
    var { data: existingBulkCommentsFromDatabase, error }: BaseQueryResponse =
      await this.commentRepository.getAllCommentsByProductId(pId);

    var preprocessedData: CommentItem[] = [],
      results: CommentItem[] = [];

    // Preprocessed the data
    preprocessedData = jsonCamelCase(
      existingBulkCommentsFromDatabase,
    ) as CommentItem[];
  ...
  exportedAllCommentsToExcelByProductId = async (
    ...
    var preprocessedData: CommentItem[] = [],
      results: ExportedCommentItemToExcel[] = [];
